<template>
  <q-page v-if="isMobile()" class="flex justify-center items-center" 
    style="
    ...
    <div class="login-container">
      ...
      <q-form @submit="onSubmit" class="q-gutter-y-md">
        <q-input v-model="email" type="email" label="Email" outlined rounded dense class="mobile-input">
          <template v-slot:append>
            <q-icon name="mail" color="grey-6" size="xs" />
          </template>
        </q-input>

        <q-input v-model="password" :type="isPwd ? 'password' : 'text'" label="Senha" outlined rounded dense
          class="mobile-input">
          <template v-slot:append>
            <q-icon :name="isPwd ? 'visibility_off' : 'visibility'" class="cursor-pointer" color="grey-6" size="xs"
              @click="isPwd = !isPwd" />
          </template>
        </q-input>
    ...
    <q-card class="login-card row">
      ...
      ">
        <div class="column items-center justify-center full-height q-pa-md">
          <h5 
            class="text-weight-semibold text-grey-8" 
          ...
          <q-form @submit="onSubmit" class="q-gutter-y-md q-px-lg">
            ...
            <div class="text-center q-gutter-y-sm">
              ...
              <p class="text-grey-7 q-mb-none">
                Não tem uma conta?
                <q-btn flat color="primary" label="Inscreva-se" no-caps class="q-px-sm" padding="none" @click="router.push('/register')" />
              </p>
            </div>
          </q-form>
        </div>
